...
const BATCHES = {
  ...
  // Batch 7 — port the APPROVED living-habitat prototype into the Home screen, driven by REAL app state.
  yap7: [
    { id: 'YV18', title: "Port Yappy's House into the Home screen as a living companion",
      ref: 'docs/prototypes/yappy-house.html',
      spec: 'Wilson APPROVED the living-habitat prototype at docs/prototypes/yappy-house.html — a pixel "dollhouse" where the chick Yappy lives a day (sleeps in a nest, wakes, wanders, works at a typewriter, waters a plant) with a day->dusk->night->dawn window, a warm light-beam + dust motes + lamp glow + vignette lighting pass. PORT IT FAITHFULLY into the app Home screen. (1) Create a React component (e.g. desktop/src/home/YappyHouse.tsx) that renders the SAME pixel scene + Yappy routines + lighting engine as the prototype: a low-res offscreen canvas (~192x128) drawn with the room/furniture/Yappy sprite + an activity director, upscaled NEAREST-NEIGHBOUR (imageSmoothingEnabled=false) onto a display canvas. Keep the EXACT cozy look — port the drawing code, do not redesign it. Also COMMIT docs/prototypes/yappy-house.html (it is already in the working tree) as the design SSOT. (2) Drive it from REAL app state, NOT the demo remote buttons: subscribe to the SAME Tauri events the app already emits (see how desktop/src/App.tsx and desktop/src/pill/YappyPill.tsx use @tauri-apps/api event listen): "recording" (true -> Yappy hurries to the desk and works / listening), "status" busy (transcribing -> thinking), "transcript" (arrived -> a brief happy/done reaction ~2s then back to idle), and "audio_level" 0..1 (feeds the beam/Yappy energy). When idle, run the ambient day-to-day director (wander / sleep / routines). (3) Day/night phase from the REAL system clock (new Date().getHours() -> dawn/day/dusk/night), NOT a fake fast cycle. (4) Show REAL stats only if trivially available from what Home/Insights already computes (words today / streak); otherwise omit — never fabricate. (5) MOUNT it as the centerpiece of the Home view in desktop/src/App.tsx (inside the `nav === "home"` block), sized responsively. Respect prefers-reduced-motion (render one calm frame, no loop). Do NOT touch the pill (YappyPill/ClassicPill), the recording/ASR/paste pipeline, Settings, or other views. Keep tsc strict-clean and match surrounding style.',
      accept: 'docs/prototypes/yappy-house.html is committed (git ls-files docs/prototypes/yappy-house.html non-empty). A Home companion component exists rendering the ported pixel engine — a nearest-neighbour canvas (grep -Rin "imageSmoothingEnabled" desktop/src/home desktop/src shows it set false in the new component; grep -REin "drawImage|createElement\\(.canvas.\\)" desktop/src/home non-empty) with Yappy + room drawing (grep -REin "yappy|nest|typewriter|drawRoom|drawYappy|skyAt|beam" desktop/src/home non-empty). It subscribes to REAL events (grep -REin "recording|audio_level|transcript|status" desktop/src/home shows event listeners) and derives day/night from the real clock (grep -REin "getHours|new Date\\(\\)" desktop/src/home non-empty). It is mounted in the Home view of App.tsx (grep -n "YappyHouse\\|<.*House" desktop/src/App.tsx non-empty). `cd desktop && npx tsc --noEmit` exits 0; `cd desktop/src-tauri && cargo build` succeeds. CI green.' },
  ],

  // Batch 8 — audit fixes from ~/Obsidian/Wilson-Brain/Notes/Yap-Code-Audit-2026-07-23.md (0 crit/0 high; 19 med/low) + the Home-height trim. Priority: privacy -> user-bug -> security -> efficiency/a11y -> hygiene.
  yap8: [
    { id: 'YV19', title: "Trim Yappy's House height so Yappy + Dictate frame together on first load",
      ref: 'docs/prototypes/yappy-house.html',
      spec: 'The Home view mounts <YappyHouse/> (desktop/src/home/YappyHouse.tsx) at the top of the nav==="home" block in desktop/src/App.tsx, but the display canvas is tall, so on a normal-height window the user lands scrolled below Yappy and must scroll up to see the scene. TRIM the DISPLAYED height so the full room + Yappy + the house stat row fit above the fold and the Dictate button is reachable without a long scroll. Do it in the component or its stylesheet (find the .yappy-house / .yh-frame / .yh-stage styles) by bounding the display height (e.g. a max-height on the frame with the canvas scaled to fit) WITHOUT distorting the pixel aspect — keep it crisp, nearest-neighbour, 3:2, and keep the WHOLE scene visible (never crop Yappy or the window). Frontend only; do not change the canvas drawing engine or any Rust. Keep tsc strict-clean and match surrounding style.',
      accept: 'The rendered house height is bounded so it fits above the fold (grep -REin "max-height|maxHeight" desktop/src shows a bound added on the house frame/stage), aspect preserved (no stretch), full scene still renders. `cd desktop && npx tsc --noEmit` exits 0; `cd desktop/src-tauri && cargo build` succeeds. CI green.' },

    { id: 'YV20', title: 'Privacy: keep transcript text out of the log, clean up leaked WAVs, take HuggingFace offline (audit M2/M3/M4)',
      ref: 'docs/OPEN-SOURCE-ROADMAP.md',
      spec: 'Fix three privacy leaks that contradict the app promise "nothing leaves this machine" (code audit M2/M3/M4). (M2) NEVER log transcript bodies: in desktop/src-tauri/src/lib.rs and asr.rs stop writing raw transcript text into the rotating log (data_dir/logs/yap.log) — the log::info that includes rejected/degenerate ASR output as {raw_text:?} (lib.rs), and asr.rs error strings that embed raw={resp} / stdout= transcript payloads. Log lengths/counts only, never the text. (M3) Stop leaking raw voice WAVs on error: in stop_and_transcribe (lib.rs) the captured wav is deleted only on the success/gate paths; on ASR or DB failure the closure returns Err WITHOUT deleting rec.wav_path, permanently leaving a .wav of the user voice in the recordings dir. Delete the wav on the Err path too (lift wav_path out of the closure OR use an RAII drop-guard that unlinks unless disarmed on success), also delete before the "Audio too short" Err in record.rs, and add a startup sweep that removes stale *.wav from the recordings dir. (M4) Take HuggingFace offline: in asr.rs apply_env (and the asr_worker.py env block) always set HF_HUB_DISABLE_TELEMETRY=1, and set HF_HUB_OFFLINE=1 whenever the model snapshot already exists in the HF cache (fall back to online only for the first download) so warm launches make NO huggingface.co calls. Do not break first-run model download, the never-lose-text, or never-block-paste guards. Add Rust #[test]s where pure (the stale-wav sweep predicate and/or a log-redaction helper).',
      accept: 'No transcript body reaches the log (grep -REn "raw_text:\\?|raw=\\{resp\\}|stdout=\\{" desktop/src-tauri/src returns nothing that logs transcript text, or it is redacted); the wav is removed on the Err path and a startup sweep exists (grep -Ein "drop|unlink|remove_file|sweep|stale" desktop/src-tauri/src/lib.rs desktop/src-tauri/src/record.rs shows error-path cleanup + a recordings sweep); HF offline+telemetry env set (grep -Ein "HF_HUB_OFFLINE|HF_HUB_DISABLE_TELEMETRY" desktop/src-tauri/src/asr.rs non-empty). `cd desktop/src-tauri && cargo test` passes; `cd desktop && npx tsc --noEmit` exits 0. CI green.' },

    { id: 'YV21', title: 'Auto-paste only into the app you dictated into (audit M1)',
      ref: 'docs/OPEN-SOURCE-ROADMAP.md',
      spec: 'Fix a wrong-target paste (code audit M1). source_app is captured at key-release, but the auto-paste (synthesized Cmd+V) runs 1-3s later inside the spawned thread after ASR, and focus::should_auto_paste() only checks that the current frontmost element is a text field — it never confirms the frontmost APP is still the one dictated into. So if the user switches apps during the ASR delay, the transcript is auto-pasted into the WRONG app (a privacy leak). Fix: capture the frontmost app identity (name/bundle/PID via focus.rs) at record start or reuse source_app, and IMMEDIATELY before synthesizing Cmd+V compare it to the CURRENT frontmost app; on mismatch, fall back to clipboard-only (copy but do NOT paste) and surface a gentle status like "Copied — you switched apps, so I did not paste". Never paste into a different app than the one dictated into. Keep the never-lose-text guarantee (always copy first). Add a pure #[test] for the compare/decision helper.',
      accept: 'The paste path compares the dictation-time frontmost app to the current one before synthesizing Cmd+V and falls back to clipboard-only on mismatch (grep -Ein "frontmost|source_app|same_app|mismatch|switched" desktop/src-tauri/src/lib.rs desktop/src-tauri/src/paste.rs shows the guard before the paste; a pure decision helper with a #[test]). `cd desktop/src-tauri && cargo test` passes; `cd desktop && npx tsc --noEmit` exits 0. CI green.' },

    { id: 'YV22', title: 'Persist EVERY Settings control (audit M6 — the top user-facing bug)',
      ref: 'docs/OPEN-SOURCE-ROADMAP.md',
      spec: 'Fix silent settings data-loss (code audit M6). In desktop/src/App.tsx only pillStyle, dictationMode, and cleanupLevel call saveSettings(); every other control only calls setSettings() (local React state) and is NEVER persisted — denoise, autoPaste, language, keepCmdShiftV, speedProfile/model, voice model, and the push-to-talk binding chips + "Set shortcut" key-capture (via applyBinding). Users therefore lose those changes on quit, and remapping the primary hold-to-talk shortcut appears to do nothing. Fix with ONE consistent pattern: either make every control persist via saveSettings({ ...settings, <field> }) like the three that already do, OR add a single debounced effect that persists settings whenever it changes. ALSO make applyBinding both persist the new binding AND actually re-bind the PTT engine in-session (call the existing backend set_binding command) so a remapped shortcut works immediately. Do not double-save or regress the three controls that already work. Every setting must round-trip across quit/relaunch.',
      accept: 'Every settings control persists (grep -n "saveSettings" desktop/src/App.tsx shows it wired for denoise, autoPaste, language, model, and the PTT binding/applyBinding — not only the original three — OR a single debounced persist-on-change effect exists) and applyBinding calls the backend rebind command. `cd desktop && npx tsc --noEmit` exits 0; `cd desktop/src-tauri && cargo build` succeeds. CI green.' },

    { id: 'YV23', title: 'Security surface: remove dylib-injection entitlements + dead Tauri plugins/capabilities (audit M5/M7)',
      ref: 'docs/OPEN-SOURCE-ROADMAP.md',
      spec: 'Shrink the attack surface before wider distribution (code audit M5 + M7). (M5) In desktop/src-tauri/Entitlements.plist REMOVE com.apple.security.cs.allow-dyld-environment-variables and com.apple.security.cs.disable-library-validation from the MAIN APP — they let DYLD_INSERT_LIBRARIES inject a dylib into a non-sandboxed process that holds Accessibility trust and synthesizes keystrokes (a TCC-inheritance privilege escalation). MLX/JIT runs in the SEPARATE Python child with its own signature, so the main app does not need these; setting env vars for a child process via Command.env() does NOT require the dyld entitlement. The app must still codesign with hardened runtime, launch, dictate, and auto-paste. (M7) Remove three registered-but-never-used Tauri plugins — tauri_plugin_opener::init(), tauri_plugin_shell::init(), and tauri_plugin_store (in lib.rs) — folders/URLs open via std::process::Command("open") and settings persist via SQLite, not these plugins. Also remove their deps in Cargo.toml and the matching capability grants (opener:default, shell:allow-open, store:default) in capabilities/default.json. One coherent change; smoke-test that it still builds.',
      accept: 'Entitlements.plist no longer grants dylib injection (grep -Ein "allow-dyld-environment-variables|disable-library-validation" desktop/src-tauri/Entitlements.plist returns nothing); the three plugins are gone from lib.rs, Cargo.toml, and capabilities/default.json (grep -REin "plugin_opener|plugin_shell|plugin_store|shell:allow-open" desktop/src-tauri returns nothing). `cd desktop/src-tauri && cargo build` succeeds; `cd desktop && npx tsc --noEmit` exits 0. CI green.' },

    { id: 'YV24', title: 'Always-on-app quality: idle-throttle the canvases, honor Reduce Motion, a11y + listener-leak fixes (audit M9/M8/L8/L2)',
      ref: 'docs/OPEN-SOURCE-ROADMAP.md',
      spec: 'Frontend quality for an always-visible utility (code audit M9/M8/L8/L2). (M9) Add idle/dirty gating to the requestAnimationFrame loops in desktop/src/home/YappyHouse.tsx and desktop/src/pill/YappyPill.tsx: when idle/sleepy AND audio_level ~0 AND no hops/sparkles in flight, skip the full redraw (still schedule the next rAF) or drop to ~15-20fps while idle — keep subtle idle drift/breathing perceptible (throttle, not a freeze). (M8) Make BOTH pills honor prefers-reduced-motion like YappyHouse already does: when reduce is set, render one calm static frame (mouth closed, level 0) instead of the 60fps loop. (L8) a11y: add aria-current="page" to the active sidebar nav button in App.tsx, and expose the live Mood value to assistive tech (drop aria-hidden on the yh-stats Mood or wrap it in an aria-live region). (L2) Fix the async listener-leak in SIX files (desktop/src/App.tsx, home/YappyHouse.tsx, pill/YappyPill.tsx, pill/ClassicPill.tsx, Onboarding.tsx, float-main.tsx): the effects do listen(...).then(u=>unsubs.push(u)) with a synchronous cleanup that can run before the promise resolves, leaking native listeners (StrictMode double-mount + Home-tab remounts). Use a disposed flag in each: let dead=false; listen(...).then(u=> dead ? u() : unsubs.push(u)); and cleanup return ()=>{ dead=true; unsubs.forEach(u=>u()); }. Frontend only; keep tsc strict-clean.',
      accept: 'Both pills gate on reduced motion (grep -Ein "prefers-reduced-motion|reduce" desktop/src/pill/YappyPill.tsx desktop/src/pill/ClassicPill.tsx non-empty); the rAF loops have an idle/throttle gate (grep -Ein "dirty|idle|throttle|skip" desktop/src/pill/YappyPill.tsx desktop/src/home/YappyHouse.tsx non-empty); aria-current present (grep -n "aria-current" desktop/src/App.tsx non-empty); the six listener effects use a disposed-flag guard (grep -Rn "dead" desktop/src/App.tsx desktop/src/home/YappyHouse.tsx desktop/src/pill/YappyPill.tsx desktop/src/pill/ClassicPill.tsx desktop/src/Onboarding.tsx desktop/src/float-main.tsx shows the guard). `cd desktop && npx tsc --noEmit` exits 0. CI green.' },

    { id: 'YV25', title: 'Dead-code & dependency hygiene + graceful startup (audit M10/M11/L1/L3/L4/L5/L6/L7)',
      ref: 'docs/OPEN-SOURCE-ROADMAP.md',
      spec: 'Pure cleanup, zero happy-path runtime change (code audit batch 5). DELETE dead files: "desktop/src/home/YappyHouse 2.tsx" (byte-identical Finder duplicate, imported nowhere), desktop/src/pill/fold.ts and desktop/src/pill/foldPlayer.ts (orphaned rejected origami-pill code), and prune the origami section of desktop/src/pill/README.md. REMOVE the four compiler-confirmed dead Rust fns: ping_daemon (asr.rs), microphone_authorized (mic_auth.rs), paste_frontmost (paste.rs), is_hands_free (ptt_macos.rs). REPLACE the misleading ptt_macos.rs branch `if keycode==... || true { handle_combo_edge(...) }` with a direct call handle_combo_edge(state, fn_down, control); (behaviour identical). REMOVE the 7 unused @tauri-apps/plugin-* npm packages from desktop/package.json (KEEP @tauri-apps/api and @tauri-apps/plugin-updater). HARDEN two startup panics into graceful degradation: lib.rs Database::open(...).expect(...) -> handle the Err (retry once with backoff, or fall back / surface a dialog — do NOT panic), and the ptt_macos.rs thread-spawn .expect -> log the error and continue so the app runs with the tray toggle / secondary shortcut. DOCUMENT at-rest storage (L1): add one line to the README (or a PRIVACY/SECURITY note) stating Yap stores transcripts in local SQLite and relies on macOS FileVault for at-rest protection. Do not remove anything still referenced; the build must stay green with FEWER warnings.',
      accept: 'Orphan files gone (test ! -f "desktop/src/home/YappyHouse 2.tsx" && test ! -f desktop/src/pill/fold.ts && test ! -f desktop/src/pill/foldPlayer.ts); the four dead Rust fns removed (grep -REn "fn ping_daemon|fn microphone_authorized|fn paste_frontmost|fn is_hands_free" desktop/src-tauri/src returns nothing); no "|| true" combo branch (grep -n "|| true" desktop/src-tauri/src/ptt_macos.rs returns nothing); only plugin-updater remains among @tauri-apps/plugin-* in desktop/package.json (grep -c "@tauri-apps/plugin-" desktop/package.json equals 1); no bare .expect on Database::open (grep -n "Database::open" desktop/src-tauri/src/lib.rs shows no .expect). `cd desktop/src-tauri && cargo build` succeeds; `cd desktop && npx tsc --noEmit` exits 0. CI green.' },
  ],
};
