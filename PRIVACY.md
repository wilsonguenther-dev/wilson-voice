...
That's the complete list. Yap sends **no** analytics, telemetry, crash reports, usage data, or personal information to us or anyone else. There is a **Send crash report to Wilson** button in Settings, and it is not an exception to that: it writes a diagnostics file to your Desktop, shows you every line of it first, and then hands it to *your* mail app — Yap has no way to upload it, and nothing is sent unless you press send in your own email.
...
- You can **delete any transcript**, **clear your entire history**, and **export diagnostics** at any time from within the app. Diagnostic logs contain **no transcript text** — and since the crash-report button can put those logs in a file you might email, that is enforced rather than intended. Every line is run through a redaction pass before it is packed: file paths, your account name, email addresses, long tokens and long numbers are replaced with a visible marker, and then the line itself has to be, in full, a message Yap ships inside itself — one of its own log messages, or a system error string from a fixed list — with only values like numbers and file names filling the gaps. Anything else is replaced with a marker and a count. A sentence you spoke is never one of Yap's own messages, however short it is and wherever in the app it was written down, and a number you spoke is not a value Yap put there. Yap shows you the redacted contents before it writes anything, and the bundle never contains a recording, a database or a transcript.
