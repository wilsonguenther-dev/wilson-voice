...
use wilson_voice_lib::meetings::{
    self, MeetingSegment, MIC_SPEAKER_LABEL, MIC_TRACK, SYSTEM_SPEAKER_LABEL, SYSTEM_TRACK,
};
...
fn mixed_tracks_render_as_one_interleaved_me_them_list() {
    let lines = meetings::render_transcript(&conversation());
...
fn the_output_is_a_conversation_not_two_stacked_columns() {
    let tracks: Vec<i64> = meetings::render_transcript(&conversation())
        .iter()
        .map(|l| l.track)
        .collect();
    assert_eq!(
...
/// The label is derived from the track, in one place, for both surfaces.
#[test]
fn labels_come_from_the_track_and_an_unknown_track_is_not_you() {
    assert_eq!(meetings::speaker_label(MIC_TRACK), "Me");
    assert_eq!(meetings::speaker_label(SYSTEM_TRACK), "Them");
    // `track` is an unconstrained INTEGER column. A value no writer in this
    // codebase produces must still render, and it certainly is not the mic.
    assert_eq!(meetings::speaker_label(7), SYSTEM_SPEAKER_LABEL);
}
